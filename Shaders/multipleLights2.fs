...
vec4 mixedColor = mix(colorText1, colorText2, 0.7); // 30% de la textura 1 y 70% de la textura 2

vec3 calculateDirectionalLight(Light light, vec3 direction){
	// Ambient
    vec3 ambient  = light.ambient * vec3(mixedColor);
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * vec3(mixedColor));
    ...
    vec3 specular = light.specular * (spec * vec3(mixedColor));  
...
void main()
{
	if(mixedColor.a < 0.1)
		discard;
    color = vec4(calculateDirectionalLight(directionalLight.light, directionalLight.direction) + calculatePointLights() + calculateSpotLights(), mixedColor.a);
}